        ...
        <div className="flex gap-10 mb-10 overflow-x-auto">
          {[
            ...
            <Link key={tab.id} href={`/?program=${program}&tab=${tab.id}&year=${year}&quarter=${quarter}&month=${month}&theme=${theme}`}
              className={`pb-3 text-sm font-black tracking-widest transition-all border-b-4 whitespace-nowrap ${activeTab === tab.id ? 'border-springGreen' : 'border-transparent hover:opacity-70'}`}
        ...
        {(activeTab === 'onboarding' || activeTab === 'eop') && (
          <section className="p-10 rounded-3xl shadow-2xl border-t-8 mt-4" style={{ backgroundColor: t.cardBg, borderColor: colors.turquoise }}>
            <h3 className="text-2xl font-black mb-2 uppercase tracking-tight flex items-end gap-3" style={{ color: t.textMain }}>
              KEY SENTIMENT INSIGHTS <span className="text-sm normal-case tracking-normal opacity-70 mb-1">(top-box scoring)</span>
            </h3>
