		...
		>
			...
			>
				...
				<nav className="flex flex-col gap-2">
					{['AiCE', 'Virtual Assistant', 'Professional Foundations'].map(
						(p) => (
							<Link
								key={p}
								href={`/?program=${p}&tab=${activeTab}&year=${year}&quarter=${quarter}&month=${month}&theme=${theme}`}
								...
								style={{
									borderColor:
										program === p ? colors.springGreen : 'transparent',
								}}
							>
								{p.toUpperCase()}
							</Link>
			...
			<main className="flex-1 p-10 overflow-y-auto relative z-10">
				...
				>
					<div className="mb-6 md:mb-0">
						...
						>
							...
							<span>{tabDisplayMap[activeTab]}</span>
						</h2>
						<p
					...
					<div className="flex flex-col items-end gap-3">
						<div className="flex gap-2 items-center">
							<Link
								href={`/?program=${program}&tab=${activeTab}&year=${year}&quarter=${quarter}&month=${month}&theme=${
									isDark ? 'light' : 'dark'
								}&event=${encodeURIComponent(activeEvent)}`}
							...
							>
								{['2025', '2026'].map((y) => (
									<Link
										key={y}
										href={`/?program=${program}&tab=${activeTab}&year=${y}&quarter=${quarter}&month=All&theme=${theme}`}
							...
							>
								{['S1', 'S2', 'S3'].map((q) => (
									<Link
										key={q}
										href={`/?program=${program}&tab=${activeTab}&year=${year}&quarter=${q}&month=All&theme=${theme}`}
										className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${
						...
						>
							<Link
								href={`/?program=${program}&tab=${activeTab}&year=${year}&quarter=${quarter}&month=All&theme=${theme}`}
							...
							{quarterMonths[quarter].map((m) => (
								<Link
									key={m.val}
									href={`/?program=${program}&tab=${activeTab}&year=${year}&quarter=${quarter}&month=${m.val}&theme=${theme}`}
									className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all ${
				...
				<div className="flex gap-10 mb-8 overflow-x-auto">
					{[
						...
						<Link
							key={tab.id}
							href={`/?program=${program}&tab=${tab.id}&year=${year}&quarter=${quarter}&month=${month}&theme=${theme}`}
							...
							<div className="relative inline-block">
								...
								>
									{uniqueEvents.map((ev) => (
										<Link
											key={ev}
											href={`/?program=${program}&tab=${activeTab}&year=${year}&quarter=${quarter}&month=${month}&theme=${theme}&event=${encodeURIComponent(
												ev
											)}`}
						...
						<>
							...
							>
								...
								>
									...
									<div
										style={{
											width: `${calcNPS(entries).ps}%`,
											backgroundColor: isDark ? '#475569' : '#CBD5E1',
										}}
									/>
									<div
										style={{
											width: `${calcNPS(entries).d}%`,
									...
									{program === 'Virtual Assistant' && (
										<>
											<InsightRow
												pct={calcTopBox(
													entries,
													'skill_va_present_professionally'
												)}
											...
											<InsightRow
												pct={calcTopBox(
													entries,
													'skill_va_communicate_effectively'
												)}
